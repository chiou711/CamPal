...
{
   private static final String EXTRA_FILENAME = "com.cwc.campal.extras.filename";
   ...
   public void onReceive(final Context context, Intent intent) 
   {
	   System.out.println("DeleteFileAlarmReceiver / _onReceive");
	    ...
	    // so, delete any file starts with CamPal_SEND and ends with txt
		...
		File[] files = folder.listFiles( new FilenameFilter() 
		{
		    ...
		    public boolean accept( final File dir,
		                           final String name ) {
//		        return name.matches( "CamPal_SEND.*\\.txt" ); // starts with LiteNote_SEND, ends with txt
//		        return name.matches( ".*\\.txt" ); // end with txt
//		        return name.matches("(CamPal_SEND.+(\\.(?i)(txt))$)" );
